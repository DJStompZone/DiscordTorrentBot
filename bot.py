...
async def downloadAnime(ctx, show, *args):
    if(ctx.channel.name == "plex-torrents"):
        show = "https://nyaa.si/?page=rss&q=" + formatShowURL(show, args) + "&c=1_2&f=0"
...
async def downloadTV(ctx, show, *args):
    if(ctx.channel.name == "plex-torrents"):    
        show = "http://127.0.0.1:9117/api/v2.0/indexers/thepiratebay/results/torznab/api?apikey=" + jackett_APIkey + "&t=search&cat=&q=" + formatShowURL(show, args)
